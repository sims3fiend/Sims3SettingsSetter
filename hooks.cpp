...
HRESULT WINAPI Present_Hook(IDirect3DDevice9* device, const RECT* pSourceRect,
    const RECT* pDestRect, HWND hDestWindowOverride, const RGNDATA* pDirtyRegion) {
    
    static bool in_present = false;
    if (in_present) {
        return Original_Present(device, pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion);
    }
