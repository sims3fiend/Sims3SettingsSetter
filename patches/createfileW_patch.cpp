...
public:
    ...
    bool Install() override {
        ...
        if (!PatchHelper::WriteDWORD(*addr1, 0x10000080, &patchedLocations, &expectedOld1)) {
            return Fail(std::format("Failed to patch at {:#010x} - expected 0x80, found {:#x}", *addr1, currentValue1));
        }
