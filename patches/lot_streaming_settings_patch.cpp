...
class LotStreamingSettingsPatch : public OptimizationPatch {
  private:
    ...
    template <typename T> bool ApplyOrDeferLiveSetting(const wchar_t* name, T value) {
        if (LiveSetting::Exists(name)) {
            // Setting exists, apply immediately
            if (!LiveSetting::Patch(name, value, &patchedLocations)) { return false; }
  ...
  public:
    ...
    bool Install() override {
        ...
        if (!ApplyOrDeferLiveSetting(L"Throttle Lot LoD Transitions", true)) { return Fail("Failed to enable 'Throttle Lot LoD Transitions'"); }

        // 2. Set "Camera speed threshold" to 5.0 to slow loading down a bit - need to recheck this tho
        if (!ApplyOrDeferLiveSetting(L"Camera speed threshold", 5.0f)) { return Fail("Failed to set 'Camera speed threshold'"); }
                    ...
                    bool applied = std::visit(
                        [&](auto&& value) -> bool {
                            ...
                            if (LiveSetting::Patch(pending.name.c_str(), value, &patchedLocations)) {
                                LOG_INFO("[LotStreamingSettingsPatch] Successfully applied deferred setting: " + Utils::WideToUtf8(pending.name));
                                return true;
                            }
                            return false;
                        },
...
REGISTER_PATCH(
    LotStreamingSettingsPatch, {.displayName = "Optimized Lot Streaming Settings",
                                   ...
                                   .technicalDetails = {"Enables 'Throttle Lot LoD Transitions' setting", "Sets 'Camera speed threshold' to 5.0 (lower = more time between stopping camera movement and lot loading)",
                                       "Reduces stutter when playing with lot counts >1", "Cross-platform compatible, just sets the live settings"}})
