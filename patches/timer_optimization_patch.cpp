...
class TimerOptimizationPatch : public OptimizationPatch {
  private:
    ...
    static void WINAPI HookedInitializeCriticalSection(LPCRITICAL_SECTION lpCriticalSection) {
        ...
        if (instance->originalInitializeCriticalSectionAndSpinCount) {
            ...
            if (instance->originalInitializeCriticalSection) { instance->originalInitializeCriticalSection(lpCriticalSection); }
            if (instance->originalSetCriticalSectionSpinCount) { instance->originalSetCriticalSectionSpinCount(lpCriticalSection, DEFAULT_SPIN_COUNT); }
        }
    }

    static BOOL WINAPI HookedInitializeCriticalSectionAndSpinCount(LPCRITICAL_SECTION lpCriticalSection, DWORD dwSpinCount) {
        if (dwSpinCount < DEFAULT_SPIN_COUNT) { dwSpinCount = DEFAULT_SPIN_COUNT; }

        if (instance && instance->originalInitializeCriticalSectionAndSpinCount) { return instance->originalInitializeCriticalSectionAndSpinCount(lpCriticalSection, dwSpinCount); }
    ...
    static DWORD WINAPI HookedSetCriticalSectionSpinCount(LPCRITICAL_SECTION lpCriticalSection, DWORD dwSpinCount) {
        ...
        if (instance && instance->originalSetCriticalSectionSpinCount) { return instance->originalSetCriticalSectionSpinCount(lpCriticalSection, dwSpinCount); }
    ...
    bool BuildCSTargets() {
        ...
        if (monoAddr) { csTargets.push_back({*monoAddr, 24000, "Mono_MethodCache"}); }
        if (browserAddr) { csTargets.push_back({*browserAddr, 20000, "Browser"}); }
        if (unkAddr) { csTargets.push_back({*unkAddr, 8000, "Unk_LookAtLater"}); }
    ...
    bool PatchSpecificCriticalSections() {
        ...
        for (const auto& config : csTargets) {
            ...
            if (!PatchHelper::IsMemoryWritable(critSec, &mbi)) {
                LOG_WARNING(std::format("[TimerOpt] Skipped {} (Invalid Memory at {:#010x}; State: {:#x}; Protect: {:#x})", config.debugName, config.address, mbi.State, mbi.Protect));
                continue;
            }
            ...
            if (prevCount != config.spinCount) {
                LOG_INFO(std::format("[TimerOpt] Tuned {} ({:#010x}) Spin: {} -> {}", config.debugName, config.address, prevCount, config.spinCount));
                successCount++;
            }
        }
  ...
  public:
    TimerOptimizationPatch()
        : OptimizationPatch("TimerOptimization", nullptr), originalInitializeCriticalSection(nullptr), originalInitializeCriticalSectionAndSpinCount(nullptr), originalSetCriticalSectionSpinCount(nullptr),
    ...
    bool Install() override {
        ...
        if (!BuildCSTargets()) { LOG_WARNING("[TimerOpt] No critical section targets available for this version"); }

        HMODULE hKernel = GetModuleHandle(L"kernel32.dll");
        if (!hKernel) return Fail("Failed to get kernel32.dll handle");

        originalInitializeCriticalSection = (InitializeCriticalSectionFn)GetProcAddress(hKernel, "InitializeCriticalSection");
        originalInitializeCriticalSectionAndSpinCount = (InitializeCriticalSectionAndSpinCountFn)GetProcAddress(hKernel, "InitializeCriticalSectionAndSpinCount");
        originalSetCriticalSectionSpinCount = (SetCriticalSectionSpinCountFn)GetProcAddress(hKernel, "SetCriticalSectionSpinCount");
        ...
        hooks = {{(void**)&originalInitializeCriticalSection, (void*)HookedInitializeCriticalSection}, {(void**)&originalInitializeCriticalSectionAndSpinCount, (void*)HookedInitializeCriticalSectionAndSpinCount},
        ...
        LOG_INFO(std::format("[TimerOpt] + Critical sections: Default spin count {} (new), tiered spin counts for {} hot sections", DEFAULT_SPIN_COUNT, csTargets.size()));
        return true;
    }
    ...
    bool Uninstall() override {
        ...
        if (!DetourHelper::RemoveHooks(hooks)) { return Fail("Failed to remove CriticalSection hooks"); }
...
REGISTER_PATCH(TimerOptimizationPatch,
    {.displayName = "Timer & Threading Optimizations",
        ...
        .technicalDetails = {"Sets system timer to 1ms via NtSetTimerResolution + timeBeginPeriod", "Hooks InitializeCriticalSection(AndSpinCount) to force 4000 spin count for new crit sections, reducing context switches",
            "Tiered critical section optimization for static critical sections (more to come, currently 3/47)", "Once enabled, static address patches will remain active even when disabled",
