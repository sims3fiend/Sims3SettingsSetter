...
class UncompressedSimTexturesPatch : public OptimizationPatch {
  ...
  public:
    ...
    bool Install() override {
        ...
        if (!setBuildOptionsUncompressedBranchAddress) { return Fail("Could not resolve setBuildOptionsUncompressedBranch address"); }
...
REGISTER_PATCH(UncompressedSimTexturesPatch, {.displayName = "Uncompressed Sim Textures",
                                                 .description = "Forces textures for sims to be uncompressed during gameplay, like they are in CAS.\n"
                                                                ...
                                                                "Specifically, when the \"Sim Detail\" setting is \"Very High\", sim textures are 2048x2048 in size: an uncompressed 2048x2048 texture occupies 16MB of "
                                                                "VRAM, whereas a compressed 2048x2048 texture would occupy only 4MB of VRAM.\n"
                                                                "When using an HQ mod (a \"GraphicsRules.sgr\" edit), sim textures may be 4096x4096 in size: an uncompressed 4096x4096 texture occupies 64MB of VRAM, "
                                                                "whereas a compressed 4096x4096 texture occupies 16MB of VRAM.\n\n\n"
                                                                "After enabling/disabling this patch, a sim's textures will not change until: their outfit is changed; or they are edited in CAS; or the game's "
                                                                "\"simCompositorCache.package\" cache is cleared (or the \"<World>_sims.package\" world-cache for custom worlds).\n\n\n"
