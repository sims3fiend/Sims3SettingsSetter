...
inline bool WriteProtectedMemory(LPVOID address, LPCVOID data, SIZE_T size, std::vector<PatchLocation>* tracker = nullptr) {
    // Validate memory is accessible before attempting to patch
    MEMORY_BASIC_INFORMATION mbi;
    if (VirtualQuery(address, &mbi, sizeof(mbi)) == 0) {
        LOG_ERROR("VirtualQuery failed for address 0x" + std::to_string(reinterpret_cast<uintptr_t>(address)));
        return false;
    }

    if (mbi.State != MEM_COMMIT) {
        LOG_ERROR("Memory at 0x" + std::to_string(reinterpret_cast<uintptr_t>(address)) + " is not committed (State: " + std::to_string(mbi.State) + ")");
        return false;
    }
    ...
    if (!VirtualProtect(address, size, PAGE_EXECUTE_READWRITE, &oldProtect)) {
        LOG_ERROR("Failed to change memory protection at 0x" + std::to_string(reinterpret_cast<uintptr_t>(address)));
        return false;
    }
    ...
    if (std::memcmp(address, data, size) != 0) {
        LOG_ERROR("Failed to verify memory write at 0x" + std::to_string(reinterpret_cast<uintptr_t>(address)));
        return false;
    }
...
inline bool IsMemoryWritable(LPVOID address, MEMORY_BASIC_INFORMATION* outMbi = nullptr) {
    ...
    constexpr DWORD WRITABLE_PROTECTIONS = PAGE_READWRITE | PAGE_EXECUTE_READWRITE | PAGE_WRITECOPY | PAGE_EXECUTE_WRITECOPY;
...
inline bool ValidateBytes(LPVOID address, LPCVOID expected, SIZE_T size) {
    ...
    constexpr DWORD READABLE_PROTECTIONS = PAGE_READONLY | PAGE_READWRITE | PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE | PAGE_WRITECOPY | PAGE_EXECUTE_WRITECOPY;

    if (mbi.State != MEM_COMMIT || (mbi.Protect & READABLE_PROTECTIONS) == 0) {
        LOG_ERROR("Memory at 0x" + std::to_string(reinterpret_cast<uintptr_t>(address)) + " is not readable (State: " + std::to_string(mbi.State) + ", Protect: " + std::to_string(mbi.Protect) + ")");
        return false;
    }
...
// Write a single byte
inline bool WriteByte(uintptr_t address, BYTE value, std::vector<PatchLocation>* tracker = nullptr, BYTE* expectedOld = nullptr) {
...
inline bool WriteBytes(uintptr_t address, const std::vector<BYTE>& bytes, std::vector<PatchLocation>* tracker = nullptr, const std::vector<BYTE>* expectedOld = nullptr) {
...
inline bool WriteDWORD(uintptr_t address, DWORD value, std::vector<PatchLocation>* tracker = nullptr, DWORD* expectedOld = nullptr) {
...
inline bool WriteWORD(uintptr_t address, WORD value, std::vector<PatchLocation>* tracker = nullptr, WORD* expectedOld = nullptr) {
...
inline bool WriteNOP(uintptr_t address, size_t count, std::vector<PatchLocation>* tracker = nullptr) {
...
inline bool RestoreAll(std::vector<PatchLocation>& locations) {
    ...
    for (auto it = locations.rbegin(); it != locations.rend(); ++it) {
        if (!WriteProtectedMemory((LPVOID)it->address, it->originalBytes.data(), it->size, nullptr)) {
            LOG_ERROR("Failed to restore patch at 0x" + std::to_string(it->address));
            success = false;
        }
    }
    if (success) { locations.clear(); }
    return success;
}
...
struct AddressInfo {
    ...
    uintptr_t GetAddressForVersion(GameVersion version) const {
        for (const auto& va : addresses) {
            if (va.version == version) { return va.address; }
        }
        return 0;
    }
    ...
    std::optional<uintptr_t> Resolve() const {
        ...
        if (g_gameVersion != GameVersion::Unknown) {
            ...
            if (addr != 0) {
                LOG_DEBUG(std::format("[{}] Using address for {}: {:#010x}", name, VERSION_NAMES[static_cast<size_t>(g_gameVersion)], addr));

                if (Validate(addr)) { return addr; }
                LOG_ERROR(std::format("[{}] Address {:#010x} failed validation for {}", name, addr, VERSION_NAMES[static_cast<size_t>(g_gameVersion)]));
            ...
            LOG_DEBUG(std::format("[{}] No address defined for {}, trying pattern scan...", name, VERSION_NAMES[static_cast<size_t>(g_gameVersion)]));
        }
        ...
        if (pattern && pattern[0] != '\0') {
            ...
            if (GetModuleInformation(GetCurrentProcess(), hModule, &modInfo, sizeof(modInfo))) {
                if (auto addr = PatchHelper::ScanPattern(static_cast<BYTE*>(modInfo.lpBaseOfDll), modInfo.SizeOfImage, pattern)) {
    ...
    bool Validate(uintptr_t addr) const {
        ...
        return std::memcmp(reinterpret_cast<void*>(addr), expectedBytes.data(), expectedBytes.size()) == 0;
    }
};
...
inline bool WriteRelativeJump(uintptr_t address, uintptr_t destination, std::vector<PatchLocation>* tracker = nullptr) {
...
inline bool WriteRelativeCall(uintptr_t address, uintptr_t destination, std::vector<PatchLocation>* tracker = nullptr) {
...
inline bool RollbackTransaction(PatchTransaction& tx) {
    if (!tx.committed && !tx.locations.empty()) { return RestoreAll(tx.locations); }
    return true;
}
...
template <typename T> inline bool GetValue(const wchar_t* name, T& outValue) {
    auto* setting = SettingsManager::Get().GetSetting(name);
    if (!setting) {
...
template <typename T> inline bool Patch(const wchar_t* name, T newValue, std::vector<PatchHelper::PatchLocation>* tracker = nullptr) {
    auto* setting = SettingsManager::Get().GetSetting(name);
    if (!setting) {
        LOG_ERROR("LiveSetting: Setting not found: " + Utils::WideToUtf8(name));
        return false;
...
inline bool SaveShaderToFile(IDirect3DPixelShader9* shader, const std::string& filename) {
    ...
    if (!GetShaderBytecode(shader, bytecode)) { return false; }
