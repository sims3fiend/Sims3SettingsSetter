...
class Handler {
  ...
  public:
    ...
    static void Log(Level level, const std::string& message, const std::source_location& loc = std::source_location::current());

    // Convenience functions
    static void Debug(const std::string& msg, const std::source_location& loc = std::source_location::current()) { Log(Level::Debug, msg, loc); }

    static void Info(const std::string& msg, const std::source_location& loc = std::source_location::current()) { Log(Level::Info, msg, loc); }

    static void Warning(const std::string& msg, const std::source_location& loc = std::source_location::current()) { Log(Level::Warning, msg, loc); }

    static void Error(const std::string& msg, const std::source_location& loc = std::source_location::current()) { Log(Level::Error, msg, loc); }

    static void Critical(const std::string& msg, const std::source_location& loc = std::source_location::current()) { Log(Level::Critical, msg, loc); }
    ...
    template <typename T> static bool IsSafeToRead(void* ptr, size_t size = sizeof(T)) {
        ...
        __try {
            volatile char dummy;
            for (size_t i = 0; i < size; i++) { dummy = static_cast<char*>(ptr)[i]; }
            return true;
        } __except (EXCEPTION_EXECUTE_HANDLER) { return false; }
    }

    // Safe exception wrapper for operations
    template <typename Func> static bool SafeExecute(Func func, const std::string& operation) {
        try {
            func();
            return true;
