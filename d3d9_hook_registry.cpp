...
template <typename THook> struct HookEntry {
    std::string name;
    THook hook;
    Priority priority;

    bool operator<(const HookEntry& other) const { return static_cast<int>(priority) < static_cast<int>(other.priority); }
};
...
template <typename THook> void SortHooks(std::vector<HookEntry<THook>>& hooks) {
    std::sort(hooks.begin(), hooks.end());
}
} // namespace

// Hooked Functions
HRESULT __stdcall Hooked_DrawIndexedPrimitive(LPDIRECT3DDEVICE9 device, D3DPRIMITIVETYPE type, INT baseVertexIndex, UINT minVertexIndex, UINT numVertices, UINT startIndex, UINT primCount) {

    DeviceContext ctx{device, false, S_OK};

    if (!Internal::ExecuteDrawIndexedPrimitiveHooks(ctx, type, baseVertexIndex, minVertexIndex, numVertices, startIndex, primCount)) { return ctx.overrideResult; }

    return Original_DrawIndexedPrimitive(device, type, baseVertexIndex, minVertexIndex, numVertices, startIndex, primCount);
}

HRESULT __stdcall Hooked_DrawPrimitive(LPDIRECT3DDEVICE9 device, D3DPRIMITIVETYPE primitiveType, UINT startVertex, UINT primitiveCount) {

    DeviceContext ctx{device, false, S_OK};

    if (!Internal::ExecuteDrawPrimitiveHooks(ctx, primitiveType, startVertex, primitiveCount)) { return ctx.overrideResult; }

    return Original_DrawPrimitive(device, primitiveType, startVertex, primitiveCount);
}

HRESULT __stdcall Hooked_SetRenderTarget(LPDIRECT3DDEVICE9 device, DWORD renderTargetIndex, IDirect3DSurface9* renderTarget) {

    DeviceContext ctx{device, false, S_OK};

    if (!Internal::ExecuteSetRenderTargetHooks(ctx, renderTargetIndex, renderTarget)) { return ctx.overrideResult; }
...
HRESULT __stdcall Hooked_SetPixelShader(LPDIRECT3DDEVICE9 device, IDirect3DPixelShader9* shader) {
    ...
    if (!Internal::ExecuteSetPixelShaderHooks(ctx, shader)) { return ctx.overrideResult; }
...
HRESULT __stdcall Hooked_SetVertexShader(LPDIRECT3DDEVICE9 device, IDirect3DVertexShader9* shader) {
    ...
    if (!Internal::ExecuteSetVertexShaderHooks(ctx, shader)) { return ctx.overrideResult; }
...
HRESULT __stdcall Hooked_SetTexture(LPDIRECT3DDEVICE9 device, DWORD stage, IDirect3DBaseTexture9* texture) {
    ...
    if (!Internal::ExecuteSetTextureHooks(ctx, stage, texture)) { return ctx.overrideResult; }

    return Original_SetTexture(device, stage, texture);
}

HRESULT __stdcall Hooked_Present(LPDIRECT3DDEVICE9 device, const RECT* sourceRect, const RECT* destRect, HWND destWindowOverride, const RGNDATA* dirtyRegion) {

    DeviceContext ctx{device, false, S_OK};

    if (!Internal::ExecutePresentHooks(ctx, sourceRect, destRect, destWindowOverride, dirtyRegion)) { return ctx.overrideResult; }
...
HRESULT __stdcall Hooked_BeginScene(LPDIRECT3DDEVICE9 device) {
    ...
    if (!Internal::ExecuteBeginSceneHooks(ctx)) { return ctx.overrideResult; }

    return Original_BeginScene(device);
}

HRESULT __stdcall Hooked_CreateTexture(LPDIRECT3DDEVICE9 device, UINT width, UINT height, UINT levels, DWORD usage, D3DFORMAT format, D3DPOOL pool, IDirect3DTexture9** texture, HANDLE* sharedHandle) {

    DeviceContext ctx{device, false, S_OK};

    if (!Internal::ExecuteCreateTextureHooks(ctx, width, height, levels, usage, format, pool, texture, sharedHandle)) { return ctx.overrideResult; }

    return Original_CreateTexture(device, width, height, levels, usage, format, pool, texture, sharedHandle);
}

HRESULT __stdcall Hooked_CreateRenderTarget(
    ...
    if (!Internal::ExecuteCreateRenderTargetHooks(ctx, width, height, format, multiSample, multisampleQuality, lockable, surface, sharedHandle)) { return ctx.overrideResult; }
...
HRESULT __stdcall Hooked_SetViewport(LPDIRECT3DDEVICE9 device, const D3DVIEWPORT9* viewport) {
    ...
    if (!Internal::ExecuteSetViewportHooks(ctx, viewport)) { return ctx.overrideResult; }
...
HRESULT __stdcall Hooked_CreatePixelShader(LPDIRECT3DDEVICE9 device, const DWORD* pFunction, IDirect3DPixelShader9** ppShader) {
    ...
    if (!Internal::ExecuteCreatePixelShaderHooks(ctx, pFunction, ppShader)) { return ctx.overrideResult; }
...
HRESULT __stdcall Hooked_CreateVertexShader(LPDIRECT3DDEVICE9 device, const DWORD* pFunction, IDirect3DVertexShader9** ppShader) {
    ...
    if (!Internal::ExecuteCreateVertexShaderHooks(ctx, pFunction, ppShader)) { return ctx.overrideResult; }
...
HRESULT __stdcall Hooked_SetPixelShaderConstantF(LPDIRECT3DDEVICE9 device, UINT startRegister, const float* pConstantData, UINT vector4fCount) {
    ...
    if (!Internal::ExecuteSetPixelShaderConstantFHooks(ctx, startRegister, pConstantData, vector4fCount)) { return ctx.overrideResult; }
...
HRESULT __stdcall Hooked_SetVertexShaderConstantF(LPDIRECT3DDEVICE9 device, UINT startRegister, const float* pConstantData, UINT vector4fCount) {
    ...
    if (!Internal::ExecuteSetVertexShaderConstantFHooks(ctx, startRegister, pConstantData, vector4fCount)) { return ctx.overrideResult; }
...
void UnregisterAll(const std::string& name) {
    ...
    auto removeByName = [&name](auto& container) { container.erase(std::remove_if(container.begin(), container.end(), [&name](const auto& entry) { return entry.name == name; }), container.end()); };
